...
alias myip='ip -4 addr show | grep -v 127.0.0.1 | grep inet | awk "{print \$NF\" = \"substr(\$2,1,index(\$2,\"/\")-1)}"'
...
alias fshare='myip && cd ~ && copyparty -c .config/copyparty/copyparty.conf'
